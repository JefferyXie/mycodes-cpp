#ifndef TEMPLATE_SPECIAL2_H
#define TEMPLATE_SPECIAL2_H
...
public:
    template <typename U,
              std::enable_if_t<!std::is_same<double,U>::value && 
                               !std::is_same<float,U>::value>* = nullptr>
    void get_as() {
        std::cout << "get as T" << std::endl;
    }
    template <typename U,
              std::enable_if_t<std::is_same<double,U>::value>* = nullptr>
    void get_as() {
        std::cout << "get as double" << std::endl;
    }
    template <typename U,
              std::enable_if_t<std::is_same<float,U>::value>* = nullptr>
    void get_as() {
...
private:
    template <typename U>
    void get_as_impl(U) {
        cout << "get_as_impl<U>(U)" << endl;
    }
    void get_as_impl(double) {
        cout << "get_as_impl(double)" << endl;
    }
    void get_as_impl(float) {
        cout << "get_as_impl(float)" << endl;
    }
};
// solution 2): use overload function
...
private:
    template <typename UT> struct type { };
    
    template <typename U>
    void get_as_impl(type<U>) {
        cout << "get_as_impl<U>(type<U>)" << endl;
    }
    void get_as_impl(type<double>) {
        cout << "get_as_impl(type<double>)" << endl;
    }
    void get_as_impl(type<float>) {
        cout << "get_as_impl(type<float>)" << endl;
    }
};

void
Run_Temp_specialization2 () 
{
    // solution 1)
    cout << "------------- solution 1) enable_if -------------" << endl;
    ...
    cout << "------ solution 2), overload with parameter ------" << endl;
    t_special_2<int> t2_i;
    int n = 10;     t2_i.get_as<int>(n);
    char c = 'a';   t2_i.get_as<char>(c);
    double o = 1.1; t2_i.get_as<double>(o);
    float f = 1.5;  t2_i.get_as<float>(f);
    ...
    cout << "----- solution 2), overload without parameter ----" << endl;
