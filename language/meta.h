...
template <>
constexpr int pow_2<0>()
{
    return 1;
}

constexpr int constexpr_pow_2(int N)
{
    return N == 0 ? 1 : (2 * constexpr_pow_2(N - 1));
}
constexpr int constexpr_pow_2_2(int N)
{
    return pow(2, N);
}
...
constexpr unsigned int factorial_safe(int N)
{
    // the exception is thrown at compile time (TODO: need to verify this really happens!!)
    return N < 0 ? throw std::invalid_argument("factorial_safe(int N) - N cannot be negative: " + std::to_string(N))
                 : factorial(N);
}
...
struct FRACTION_SUM {
    ...
    typedef FRACTION<
        FRACT_A::NUMERATOR*(FRACT_B::DENOMINATOR / internal_mcd) +
...
{
public:
    static inline void EXEC() { std::cout << "SWITCH: default "; }
};
template <>
class SWITCH<1>
{
public:
    static inline void EXEC() { std::cout << "SWITCH: 1 "; }
};
template <>
class SWITCH<2>
{
public:
    static inline void EXEC() { std::cout << "SWITCH: 2 "; }
};
...
void run_meta()
{
    //
    // pow
    static_assert(pow_2<5>() == 32, "wrong result");
    ...
    if (SAME_TYPE<std::map<int, std::string>::key_type, int>::VALUE) {
    ...
    // FRACTION
    std::cout << "\n[FRACTION]" << std::endl;
    std::cout << "FRACTION<1, 2>: " << FRACTION<1, 2>::NUMERATOR << "/" << FRACTION<1, 2>::DENOMINATOR << '\n';
    std::cout << "FRACTION<3, 6>: " << FRACTION<3, 6>::NUMERATOR << "/" << FRACTION<3, 6>::DENOMINATOR << '\n';
    typedef FRACTION<36, 54> frac_36_54;
    std::cout << "FRACTION<36, 54>: " << frac_36_54::NUMERATOR << "/" << frac_36_54::DENOMINATOR << '\n';
    typedef FRACTION<54, 36> frac_54_36;
    std::cout << "FRACTION<54, 36>: " << frac_54_36::NUMERATOR << "/" << frac_54_36::DENOMINATOR << '\n';

    //
    // FRACTION_SUM
    std::cout << "\n[FRACTION_SUM]" << std::endl;
    typedef FRACTION_SUM<FRACTION<1, 2>, FRACTION<2, 3>>::VALUE frac_sum_1;
    std::cout << "1/2 + 2/3: " << frac_sum_1::NUMERATOR << "/" << frac_sum_1::DENOMINATOR << '\n';
    typedef FRACTION_SUM<FRACTION<2, 6>, FRACTION<4, 9>>::VALUE frac_sum_2;
    std::cout << "2/6 + 4/9: " << frac_sum_2::NUMERATOR << "/" << frac_sum_2::DENOMINATOR << '\n';
    typedef FRACTION_SUM<FRACTION<-1, 2>, FRACTION<4, 6>>::VALUE frac_sum_3;
    std::cout << "-1/2 + 4/6: " << frac_sum_3::NUMERATOR << "/" << frac_sum_3::DENOMINATOR << '\n';
    ...
    std::cout << "E<0>: " << (double)e_0::NUMERATOR / e_0::DENOMINATOR << ", " << e_0::NUMERATOR << '/'
              << e_0::DENOMINATOR << '\n';
    std::cout << "E<1>: " << (double)e_1::NUMERATOR / e_1::DENOMINATOR << ", " << e_1::NUMERATOR << '/'
              << e_1::DENOMINATOR << '\n';
    std::cout << "E<2>: " << (double)e_2::NUMERATOR / e_2::DENOMINATOR << ", " << e_2::NUMERATOR << '/'
              << e_2::DENOMINATOR << '\n';
    std::cout << "E<3>: " << (double)e_3::NUMERATOR / e_3::DENOMINATOR << ", " << e_3::NUMERATOR << '/'
              << e_3::DENOMINATOR << '\n';
    std::cout << "E<4>: " << (double)e_4::NUMERATOR / e_4::DENOMINATOR << ", " << e_4::NUMERATOR << '/'
              << e_4::DENOMINATOR << '\n';
    std::cout << "E<5>: " << (double)e_5::NUMERATOR / e_5::DENOMINATOR << ", " << e_5::NUMERATOR << '/'
              << e_5::DENOMINATOR << '\n';
    std::cout << "E<10>: " << (double)e_10::NUMERATOR / e_10::DENOMINATOR << ", " << e_10::NUMERATOR << '/'
              << e_10::DENOMINATOR << '\n';

    //
    // IF
    std::cout << "\n[IF]" << std::endl;
    IF<true, int, std::string>::TYPE  i = 10;
    IF<false, int, std::string>::TYPE s = "20";
