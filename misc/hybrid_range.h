...
public:
    ...
    public:
        hybrid_iterator(Iter_O begin_o, Iter_O end_o, Iter_CO begin_co, Iter_CO end_co)
            : o_(begin_o),
              end_o_(end_o),
              co_(begin_co),
              end_co_(end_co),
        ...
        bool      operator!=(const hybrid_iterator& rhs) const { return result_ != rhs.result_; }
        Result_t& operator*() { return *result_; }
    };

    hybrid_range(Range_O&& o, Range_CO&& co)
    {
        // NOTICE: a normal way to move c-style array
        if constexpr (std::is_array_v<Range_O>) {
            std::move(std::begin(o), std::end(o), std::begin(range_o_));
        } else {
            range_o_ = std::move(o);
        }
        if constexpr (std::is_array_v<Range_CO>) {
            std::move(std::begin(co), std::end(co), std::begin(range_co_));
        } else {
            range_co_ = std::move(co);
        }
    }

    auto begin()
    {
        return hybrid_iterator<decltype(std::begin(range_o_)), decltype(std::begin(range_co_))>{
            std::begin(range_o_), std::end(range_o_), std::begin(range_co_), std::end(range_co_)};
    }
    auto end()
    {
        return hybrid_iterator<decltype(std::begin(range_o_)), decltype(std::begin(range_co_))>{
            std::end(range_o_), std::end(range_o_), std::end(range_co_), std::end(range_co_)};
    }
};

auto get_range_vec()
{
    std::vector<st_2> v_2 = {st_2{1}, st_2{2}};
    std::vector<st_3> v_3 = {st_3{3}, st_3{4}, st_3{5}};
    return hybrid_range<decltype(v_2), decltype(v_3)>(std::move(v_2), std::move(v_3));
}
auto get_range_arr()
{
    st_2 arr_2[] = {st_2{1}, st_2{2}};
    st_3 arr_3[] = {st_3{3}, st_3{4}, st_3{5}};
    static_assert(std::is_same_v<decltype(arr_2), st_2[2]>);
    static_assert(std::is_same_v<decltype(std::move(arr_2)), st_2(&&)[2]>);
    return hybrid_range<decltype(arr_2), decltype(arr_3)>(std::move(arr_2), std::move(arr_3));
}
auto get_range_map()
{
    std::map<int, st_2> m_2 = {{1, st_2{1}}, {2, st_2{2}}};
    std::map<int, st_2> m_3 = {{3, st_2{3}}, {4, st_2{4}}, {5, st_2{5}}};
    using Result_Type       = std::map<int, st_2>::value_type;
    return hybrid_range<decltype(m_2), decltype(m_3), Result_Type>(std::move(m_2), std::move(m_3));
...
void run_hybrid_range()
{
    ...
    for (auto& m : get_range_map()) {
        std::cout << m.first << "," << m.second.v << std::endl;
    }
