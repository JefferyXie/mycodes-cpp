...
//
// *** This may not be the optimal solution and I'm always happy to get your feedback. Thank you for reading! ***
//
...
//
// 2) get_shape_meta: a simple but important utility that tells each shape's meta data. Meta data is basically a matrix
//                    that tells occupied/non-occupied cells in the shape. Any time when we want to add a new shape to
//                    the game, just add its meta to this utility so that engine will pick it up automatically.
//
...
// Any time when a new shape is created, we just need to add its meta to this utility so that engine will pick it up
// automatically.
...
// A class that processes input data from either file or standard console.
...
public:
    ...
    bool read_line(std::vector<input_shape_t>& shape_series)
    {
        ...
        for (const auto& token : tokens) {
            ...
            if (shape != 'Q' && shape != 'Z' && shape != 'S' && shape != 'T' && shape != 'I' && shape != 'L' &&
                shape != 'J') {
                error(input, token);
                return false;
            }
    ...
    int erase_rows(internal_state_t& state, int min_idx_updated_row)
    {
        ...
        while (true) {
            ...
            if (occupied == max_width_) {
                // reset if this row is fully occupied
            ...
            // relocate current non-fully occupied row
