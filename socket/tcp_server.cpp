...
void tcp_server::communicate(int sock_server)
{
    ...
    while (1) {
        ...
        printf(
            "New client connected from port %d and IP %s\n", ntohs(addr_client.sin_port),
        ...
        printf(
            "\t**getsockname - connected client is %s:%d\n", inet_ntoa(addr_info.sin_addr), ntohs(addr_info.sin_port));
        ...
        printf(
            "\t**getpeername - connected client is %s:%d\n", inet_ntoa(addr_info.sin_addr), ntohs(addr_info.sin_port));
        // end dump

        threads.emplace_back(std::thread(&tcp_server::handleclient, sock_client));
    }
    std::for_each(std::begin(threads), std::end(threads), [](std::thread& th) {
        th.join();
    });
}
