...
unsigned int generate_blink_signal(
    unsigned int len_all, const std::vector<unsigned int>& pos_ons, const std::vector<unsigned int>& pos_offs,
    std::vector<std::string>& blink_signals)
{
    ...
    std::unordered_set<unsigned int> pos_on_hash;
    for_each(pos_ons.begin(), pos_ons.end(), [&pos_on_hash](unsigned int v) {
        pos_on_hash.insert(v);
    });
    // convert pos_offs into vector with index as position
    std::unordered_set<unsigned int> pos_off_hash;
    for_each(pos_offs.begin(), pos_offs.end(), [&pos_off_hash](unsigned int v) {
        pos_off_hash.insert(v);
    });

    std::string signal(len_all, '0');
    // keep those always on positions '1'
    for_each(pos_ons.begin(), pos_ons.end(), [&signal](unsigned int pos) {
    ...
    while (count++ < count_possible) {
        ...
        while (j--) {
            ...
            if (pos_on_hash.end() != pos_on_hash.find(pos) || pos_off_hash.end() != pos_off_hash.find(pos)) {
                continue;
            }
...
void run_generate_blink_signals()
{
    ...
    std::vector<unsigned int> pos_ons  = {};
    std::vector<unsigned int> pos_offs = {};
    ...
    auto lam_blink = [&]() {
        ...
        std::cout << "\n# of signals: " << generate_blink_signal(len_all, pos_ons, pos_offs, blink_signals)
                  << std::endl;
        for_each(blink_signals.begin(), blink_signals.end(), [](const std::string& s) {
            std::cout << '\t' << s << '\n';
        });
    };
    lam_blink();
