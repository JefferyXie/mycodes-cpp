...
//
// https://techsauce.medium.com/finding-the-length-of-the-longest-substring-without-repeating-characters-0a76ae287bae
//
std::string substring_longest_unique(char* str)
{
    ...
    // Start from the second character. First character is already processed (cur_len and max_len are initialized as 1,
    // and visited[str[0]] is set
    for (int i = 1; i < (int)strlen(str); i++) {
        prev_index = visited[str[i] - 'a'];

        // If the currentt character is not present in the already processed substring or it is not part of the current
        // NRCS, then do cur_len++
        if (prev_index == -1 || i - cur_len > prev_index) {
            cur_len++;
        }
        // If the current character is present in currently considered NRCS, then update NRCS to start from the next
        // character of previous instance.
        else {
            // Also, when we are changing the NRCS, we should also check whether length of the previous NRCS was greater
            // than max_len or not.
            if (cur_len > max_len) {
                max_len      = cur_len;
