...
//
// solution 1)
void inc_last(int last, std::vector<int>& candies, const std::vector<int>& arr)
{
    candies[last] += 1;
    if (last <= 0)
        return;
...
long candies_1(int n, const std::vector<int>& arr)
{
...
long candies_2(int n, const std::vector<int>& arr)
{
    std::vector<int> candies;
    candies.resize(arr.size(), 1);
    for (int i = 1; i < n; i++) {
        if (arr[i] > arr[i - 1]) {
            ...
            for (int j = i; j > 0 && arr[j] < arr[j - 1] && candies[j] == candies[j - 1]; candies[--j] += 1) {
            }
        }
    }
    long sum = 0;
...
long candies_3(int n, const std::vector<int>& arr)
{
    std::vector<int> dp;
